...
export function emitAccountLoadedEvent(account: BusinessAccount | IndividualAccount, source?: Element) {
  console.debug(`Will fire loaded event for account: ${account.id}`, { source })
  const event = new CustomEvent<LoadedAccountEventDetail>(nsEventName('account:loaded'), {
    detail: { account },
  })
  ;(source ?? document).dispatchEvent(event)
}
