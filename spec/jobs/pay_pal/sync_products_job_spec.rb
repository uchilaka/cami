    ...
    before do
      allow_any_instance_of(described_class).to receive(:vendor_credentials) do
        OpenStruct.new(
          ...
          client_id: ENV.fetch('PAYPAL_CLIENT_ID', Rails.application.credentials.paypal&.client_id),
          client_secret: ENV.fetch('PAYPAL_CLIENT_SECRET', Rails.application.credentials.paypal&.client_secret)
        )
      end
    end
